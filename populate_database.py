...
from services.ehr_service import (
    ...
    obstetric_service,
    marital_service,
    history_present_illness_service,
    past_medical_history_service,
    family_history_service,
    medication_history_service,
    allergy_history_service,
...
async def populate_database():
    ...
    try:
        ...
        if us_patient:
            logger.info(
                f"✓ US Patient PT0001 (Maria Rodriguez) found: {us_patient[0]['first_name']} {us_patient[0]['last_name']}"
            )
        ...
        if cn_patient:
            logger.info(
                f"✓ Chinese Patient PT0011 (Wei Zhang) found: {cn_patient[0]['first_name']} {cn_patient[0]['last_name']}"
            )
        ...
        if elderly_patient:
            logger.info(
                f"✓ Elderly Patient PT0020 (Xiu Huang) found: {elderly_patient[0]['first_name']} {elderly_patient[0]['last_name']}"
            )
...
async def clear_database():
    ...
    try:
        ...
        for collection_name in collections:
            ...
            logger.info(
                f"Cleared {result.deleted_count} records from {collection_name}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--clear-only", action="store_true", help="Only clear data, don't populate"
    )
