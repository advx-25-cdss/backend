...
class Diagnosis(BaseModel):
    ...
    follow_up: str = Field(
        ..., 
        description="A clear plan for patient follow-up."
    )
...
class ClinicalPlan(BaseModel):
    ...
    medication_plan: List[Medicine] = Field(
        ...
        description="List of recommended new medications."
    )
    other_treatments: List[Treatment] = Field(
        default_factory=list, 
        description="List of non-medication treatments."
    )

# --- 2. Function to Call OpenAI API ---
def generate_clinical_plan(patient_data: dict) -> Optional[ClinicalPlan]:
    """
    ...
    system_prompt = f"""
    You are an expert AI acting as a Clinical Decision Support System respond in simplied chinese.
...
if __name__ == "__main__":
    ...
    sample_patient_data = {
        ...
        "current_medications": [
            {"medicine_name": "Metformin", "dosage": "1000mg", "frequency": "twice daily"},
            {"medicine_name": "Lisinopril", "dosage": "20mg", "frequency": "once daily"},
            {"medicine_name": "Warfarin", "dosage": "5mg", "frequency": "once daily", "notes": "For previous DVT"}
        ],
        "recent_test_results": [
            {
    ...
    if clinical_plan:
        ...
        if clinical_plan.medication_plan:
            ...
            for med in clinical_plan.medication_plan:
                print(f"  - {med.medicine_name} {med.dosage}, {med.frequency} ({med.route})")
                if med.notes:
